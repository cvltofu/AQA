...
export default class BaseElement {
    ...
    async isElemDisplayed() {
        logger.info(`Ckeck elem ${this._name} to be displayed.`);
    ...
    async getText() {
        ...
        logger.info(`Getting ${this._name} elem text ${text}.`);
        return text;
    }

    async getAttribute(attribute) {
        const attributeValue = await $(this._uniqueSelector).getAttribute(
            attribute
        );
        logger.info(`Getting ${this._name} attribute ${attribute}.`);
